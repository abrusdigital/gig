...
class gigCheckProductlistList(models.Model):
    ...
    status = fields.Selection([('done', 'Done'), ('pending', 'Pending')],default='pending')
    display_type = fields.Selection([('line_section', "Section"),('line_note', "Note")], default=False, help="Technical field for UX purpose.")
...
class gigChecklistProjectTask(models.Model):
    ...
    def _compute_progress_count(self):
        for rec in self:
            total_len = self.env['project.checklist.master.list'].sudo().search_count([('project_id','=',rec.id),('display_type','!=','line_note')])
            rec.checklist_progress = total_len
            if total_len != 0:
                completed_checklist = self.env['project.checklist.master.list'].sudo().search_count([('project_id','=',rec.id),('status','=','done')])
